...
| Compaction publish | pack/side-file Create uploads (parallel) → pre-claim fence over every output (parallel `bump_version`, cold maintainer path) → log slot Create → manifest CAS | objects: 2 requests each (upload/adopt + fence); then log + CAS. The extra fence is intentionally off the push path | `publish.rs::publish_compact_impl` |
...
| Publisher adopts existing pack/idx (rare push path) | Create 412 → pre-CAS `bump_version`; if the sweep already deleted it, re-upload local bytes | +1 request/round for each adopted object; healthy new-content push remains 5 | `publish.rs::put_immutable_create` |
| GC sweep (maintainer, cold path) | 1 cond GET (manifest) → gc/audit.pb GET ∥ 4 LISTs → follow C committed checkpoint links (sequential) → retained log GETs (L, parallel) → on deletion: fresh manifest GET → conditional DELETE batches. Stateless, no writes. Op wrapper: gc lease acquire 2 + release 1 (+ heartbeat for long units). Plan pass: gc/audit.pb GET and optional base-pack HEAD when gc enabled | steady fixture: 8 requests (C=1, L=1; pinned by `sim.rs::healthy_gc_sweep_budget`); general: 6+C+L before deletes. GCS/memory delete = D requests; S3 conditional delete = 2D (HEAD for NotFound distinction + atomic `If-Match` DELETE) | `walgit-wal/src/gc.rs::sweep`, `ops.rs::gc_lease` |
| Connectivity-lite audit (gc gate) | gc/audit.pb GET (CAS base) → Serve sync/capture manifest+roots → local walk (D18 history pack + live `.idx`; a remote base pays one-time `remote-idx` downloads) → gc/audit.pb CAS | 3 control requests plus missing idx downloads; op wrapper adds gc lease acquire/release | `walgit-git/src/audit.rs`, `ops.rs` gc-audit |
